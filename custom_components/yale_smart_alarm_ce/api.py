...
class YaleApiClient:
    ...
    async def _check_response_status(
        ...
        if resp.status == _HTTP_UNAUTHORIZED:
            _LOGGER.debug("Authentication failed (HTTP 401) on %s", resp.url.path)
            msg = "Authentication failed — please reconfigure the integration"
        ...
        if resp.status == _HTTP_PRECONDITION_FAILED:
            _LOGGER.debug("Precondition failed (HTTP 412) on %s", resp.url.path)
        ...
        if resp.status == _HTTP_CONFLICT:
            ...
            try:
                ...
                for entry in body_json.get("inhibitErrors") or []:
                    device_id = entry.get("_id")
                    code = entry.get("code", "")
                    if device_id:
                        inhibit_errors.append({"device_id": device_id, "code": code})
            except (ValueError, KeyError, TypeError, AttributeError):
                # Yale API has been observed returning non-dict JSON shapes
                # on some error paths (historical precedent on the doorbells
                # endpoint).  ``AttributeError`` covers the case where the
                # body parses as a list or other non-mapping object and the
                # ``.get()`` / ``.get(..., "")`` chain fails.
                raw = await resp.text()
                msg = f"Arm conflict (HTTP 409): {raw[:200]}"
                raise YaleArmConflictError(msg, inhibit_errors=[]) from None
            msg = f"Arm conflict: {len(inhibit_errors)} sensor(s) blocking"
            raise YaleArmConflictError(msg, inhibit_errors=inhibit_errors)
    ...
    async def _request(
        ...
        *,
        ...
        # Sum (not max) so mixed-error sequences don't exhaust the loop early.
        max_attempts = _MAX_HTTP_403_RETRIES + _MAX_NETWORK_RETRIES
            ...
            try:
                async with self._session.request(
                    ...
                    if resp.status == _HTTP_FORBIDDEN:
                        http_403_count += 1
                        if http_403_count < _MAX_HTTP_403_RETRIES:
                            delay = random.uniform(0, _HTTP_403_RETRY_BASE_DELAY ** http_403_count)
                            _LOGGER.warning(
                        ...
                        _LOGGER.debug(
                            "HTTP 403 persisted after %d attempts on %s",
                            _MAX_HTTP_403_RETRIES,
                            endpoint,
                        )
                        msg = (
                            f"Authentication failed (HTTP {resp.status}) "
                            f"after {_MAX_HTTP_403_RETRIES} attempts"
                        )
                        raise YaleAuthenticationError(msg)
                ...
                if network_error_count < _MAX_NETWORK_RETRIES:
                    delay = random.uniform(
                        0,
                        min(_NETWORK_RETRY_MAX_DELAY, _NETWORK_RETRY_BASE_DELAY ** network_error_count),
                    )
                    _LOGGER.warning(
    ...
    async def set_alarm_state(
        ...
        area_ids: list[str],
        *,
        inhibit_devices: list[str] | None = None,
    ) -> dict[str, Any]:
        """Set the arm state for specific areas.

        Args:
            alarm_id: The alarm identifier.
            state: Target state command (CMD_DISARM, CMD_PARTIAL_ARM, CMD_FULL_ARM).
            area_ids: List of area IDs to arm/disarm.
            inhibit_devices: Optional device IDs to bypass (ignore open sensors).
        """
        endpoint = f"/alarms/{alarm_id}/state/{state}"
        data: dict[str, Any] = {"areaIDs": area_ids}
        if inhibit_devices is not None:
            data["inhibitDevices"] = inhibit_devices
